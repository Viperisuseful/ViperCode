...
const RelayClientBearerAuthorization = HttpApiSecurity.http({ scheme: "bearer" }).pipe(
  HttpApiSecurity.annotate(
    OpenApi.Description,
    "Clerk session or OAuth bearer token for the signed-in Viper Connect user.",
  ),
);
...
export const RelayPublicClientId = Schema.Literals(["viper-mobile", "viper-web"]);
export type RelayPublicClientId = typeof RelayPublicClientId.Type;
export const RelayMobileClientId = "viper-mobile" as const;
export const RelayWebClientId = "viper-web" as const;
...
export const RelayApi = HttpApi.make("RelayApi")
  ...
  .annotate(OpenApi.Title, "Viper Code Relay API")
