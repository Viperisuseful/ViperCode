      ...
      Effect.tryPromise({
        try: input.readClerkToken,
          ...
          new ManagedRelaySessionError({
            message: "Could not obtain the Viper Connect session token.",
            cause,
          }),
      }),
  };
  ...
  return session.readClerkToken().pipe(
    Effect.flatMap((token) =>
      token
        ...
        : Effect.fail(
            new ManagedRelaySessionError({
              message: "The Viper Connect session token is unavailable.",
            }),
          ),
    ),
  );
...
function requireClerkToken(
  ...
  if (!session || session.accountId !== accountId) {
    return Effect.fail(
      new ManagedRelaySessionError({
        message: "Sign in to Viper Connect before loading relay data.",
      }),
    );
  }
...
export function readManagedRelaySnapshotState<A>(
  ...
  if (result._tag === "Failure") {
    ...
    error = cause instanceof Error ? cause.message : "Could not load Viper Connect data.";
  }
  return {
