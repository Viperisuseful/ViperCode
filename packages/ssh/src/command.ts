...
import type { DesktopSshEnvironmentTarget, DesktopUpdateChannel } from "@vipercode/contracts";
...
const PUBLISHABLE_VIPER_VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/u;
...
export function resolveRemoteViperCliPackageSpec(input: {
  readonly appVersion: string;
  readonly updateChannel: DesktopUpdateChannel;
  readonly isDevelopment?: boolean;
}): string {
  const appVersion = input.appVersion.trim();
  if (!input.isDevelopment && PUBLISHABLE_VIPER_VERSION_PATTERN.test(appVersion)) {
    return `vipercode@${appVersion}`;
  }

  if (input.isDevelopment) {
    return "vipercode@nightly";
  }

  return input.updateChannel === "nightly" ? "vipercode@nightly" : "vipercode@latest";
}
