...
import {
  ...
  resolveRemoteViperCliPackageSpec,
  runSshCommand,
} from "./command.ts";
...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote vipercode package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteViperCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "vipercode@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteViperCliPackageSpec({
