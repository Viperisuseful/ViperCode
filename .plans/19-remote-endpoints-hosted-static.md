...
- users can generate a hosted pairing link for `app.vipercode.app`
...
- `https://app.vipercode.app` cannot reliably call `http://...` or `ws://...` endpoints because browsers block mixed content.
...
- Generate `app.vipercode.app` links only from endpoints that are likely hosted-browser compatible, or show a warning with an explicit fallback.
...
```text
https://app.vipercode.app/pair?host=<encoded endpoint httpBaseUrl>#token=<one-time token>
```
...
- HTTP private-network endpoints are marked incompatible with `app.vipercode.app`.
...
- `app.vipercode.app` works as a real client shell.
...
- `app.vipercode.app` can pair a reachable HTTPS backend and reconnect after reload.
- A saved environment can be used without any backend at `app.vipercode.app`.
...
- create endpoint with `provider: "viper-tunnel"`
- surface tunnel status
- provide stable HTTPS URL
- use existing backend pairing/session auth
- never bypass server auth

The tunnel fabric can later be Pipenet-derived, Tailscale-derived, or another reverse tunnel implementation. The rest of Viper Code should only see an `AdvertisedEndpoint`.
