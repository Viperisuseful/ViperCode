...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `viper` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@vipercode/shared/git`, `@vipercode/shared/DrainableWorker`) — no barrel index.
