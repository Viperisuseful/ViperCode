...
This guide is for people who want to use more than one Codex account in Viper Code.
...
- both accounts can see the same Viper Code/Codex sessions
...
In Viper Code Settings, name it something obvious:
...
In Viper Code Settings, add another Codex provider:
...
Viper Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, Viper Code treats it
