...
Viper Code has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.viper/userdata/logs/server.trace.ndjson`).
...
```bash
export VIPERCODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export VIPERCODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export VIPERCODE_OTLP_SERVICE_NAME=viper-local
```

Optional:

```bash
export VIPERCODE_TRACE_MIN_LEVEL=Info
export VIPERCODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `VIPERCODE_OTLP_*`.

macOS app bundle example:

```bash
VIPERCODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
VIPERCODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
VIPERCODE_OTLP_SERVICE_NAME=viper-desktop \
"/Applications/Viper Code.app/Contents/MacOS/Viper Code"
```
...
```bash
tail -f "$VIPERCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$VIPERCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$VIPERCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `viper-local`, `viper-dev`, or `viper-desktop`
...
- `VIPERCODE_OTLP_TRACES_URL` was not set
...
- `VIPERCODE_TRACE_FILE`: override trace file path
- `VIPERCODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `VIPERCODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `VIPERCODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `VIPERCODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `VIPERCODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `VIPERCODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `VIPERCODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `VIPERCODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `VIPERCODE_OTLP_SERVICE_NAME`: service name, default `viper-server`
