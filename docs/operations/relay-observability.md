...
- `viper-code-relay-traces-prod`, an OpenTelemetry trace dataset for Worker requests
- `viper-code-relay-otel-ingest-prod`, a dataset-scoped ingest token bound to the Worker
- `viper-code-relay-recent-spans-prod`, a view of recent request and endpoint spans

Alchemy stages append their sanitized stage name to isolate resources, for example
`viper-code-relay-traces-dev-julius` for a personal stage.
