...
- Reads the shared production Viper Connect relay URL and Clerk client configuration before packaging clients.
...
- Publishes the CLI package (`apps/server`, npm package `viper`) with OIDC trusted publishing from the same workflow file:
...
## Viper Connect relay deployment
...
```sh
vp run --filter vipercode-relay deploy -- --stage "$USER" --env-file .env.local
```
...
- `VIPERCODE_WEB_ROUTER_URL`: defaults to `https://app.vipercode.app`.
- `VIPERCODE_WEB_LATEST_DOMAIN`: defaults to `latest.app.vipercode.app`.
- `VIPERCODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.vipercode.app`.

Required Vercel domains:

- `app.vipercode.app`: the router domain users open, updated by stable releases.
- `latest.app.vipercode.app`: channel alias updated by stable releases.
- `nightly.app.vipercode.app`: channel alias updated by nightly releases.

The router domain uses `apps/web/vercel.ts` routes. Users opt into a channel by
visiting `/__vipercode/channel?channel=latest` or
`/__vipercode/channel?channel=nightly`; the router stores the
`vipercode_web_channel` cookie and rewrites future requests on `app.vipercode.app` to
...
through `/__vipercode/channel` on the router domain so the user's channel cookie is
...
4. Run one stable release deployment, or manually alias the current stable
   deployment, so `app.vipercode.app` points at a deployment containing the router
...
- Publishes the CLI package (`apps/server`, npm package `viper`) to the `nightly` npm dist-tag using the same nightly version.
...
- Repository slug source:
  - `VIPERCODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `VIPERCODE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `viper` (or rename package first if needed).
