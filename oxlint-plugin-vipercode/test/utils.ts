...
export const createOxlintRuleHarness = (
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "vipercode-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-vipercode", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "vipercode", specifier: pluginPath }],
