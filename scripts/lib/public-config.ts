...
export interface ViperCodePublicConfig {
...
export function loadRepoEnv({
  ...
  return {
    ...
    ...(config.clerkPublishableKey
      ? {
          VIPERCODE_CLERK_PUBLISHABLE_KEY: config.clerkPublishableKey,
    ...
    ...(config.clerkJwtTemplate
      ? {
          VIPERCODE_CLERK_JWT_TEMPLATE: config.clerkJwtTemplate,
    ...
    ...(config.clerkCliOAuthClientId
      ? {
          VIPERCODE_CLERK_CLI_OAUTH_CLIENT_ID: config.clerkCliOAuthClientId,
        }
      : {}),
    ...(config.relayUrl
      ? {
          VIPERCODE_RELAY_URL: config.relayUrl,
          VITE_VIPERCODE_RELAY_URL: config.relayUrl,
        }
      : {}),
    ...(config.mobileOtlpTracesUrl
      ? {
          VIPERCODE_MOBILE_OTLP_TRACES_URL: config.mobileOtlpTracesUrl,
          EXPO_PUBLIC_OTLP_TRACES_URL: config.mobileOtlpTracesUrl,
        }
      : {}),
    ...(config.mobileOtlpTracesDataset
      ? {
          VIPERCODE_MOBILE_OTLP_TRACES_DATASET: config.mobileOtlpTracesDataset,
          EXPO_PUBLIC_OTLP_TRACES_DATASET: config.mobileOtlpTracesDataset,
        }
      : {}),
    ...(config.mobileOtlpTracesToken
      ? {
          VIPERCODE_MOBILE_OTLP_TRACES_TOKEN: config.mobileOtlpTracesToken,
...
export function resolvePublicConfig(...sources: readonly Environment[]): ViperCodePublicConfig {
  return {
    ...
    mobileOtlpTracesDataset: firstNonEmpty(
      sources,
      "VIPERCODE_MOBILE_OTLP_TRACES_DATASET",
      "EXPO_PUBLIC_OTLP_TRACES_DATASET",
    ),
    mobileOtlpTracesToken: firstNonEmpty(
      sources,
      "VIPERCODE_MOBILE_OTLP_TRACES_TOKEN",
