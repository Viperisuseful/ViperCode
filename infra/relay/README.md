...
> [!WARNING]
> Viper Connect is currently in private beta. Join the waitlist in the app under Settings > Viper Connect.

The relay is the hosted control plane for Viper Connect. It helps clients discover and connect to
...
The relay is intentionally not in the hot path for normal Viper Code traffic. After a client connects,
regular API and WebSocket traffic goes directly between that client and the selected environment.
See the [Viper Connect architecture overview](../../docs/cloud/viper-code-connect-auth-flow.html) for the larger system
design.
...
- Linking Viper Code environments to a cloud account.
...
```sh
vp run --filter vipercode-relay deploy
```
...
```sh
vp run --filter vipercode-relay deploy -- --stage prod
vp run --filter vipercode-relay deploy -- --env-file .env.local
```
...
- [Viper Connect Clerk Setup](../../docs/cloud/viper-connect-clerk.md) for Clerk keys, JWT templates, and waitlist
  setup.
- [Relay Observability](../../docs/relay-observability.md) for deployment tracing and diagnostics.
- [Viper Connect Architecture Overview](../../docs/cloud/viper-code-connect-auth-flow.html) for the full link,
