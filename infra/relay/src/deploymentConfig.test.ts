...
describe("relayResourceNameForStage", () => {
  it("isolates production and personal stages", () => {
    expect(relayResourceNameForStage("viper-code-relay-traces", "prod")).toBe(
      "viper-code-relay-traces-prod",
    );
    expect(relayResourceNameForStage("viper-code-relay-traces", "dev_julius")).toBe(
      "viper-code-relay-traces-dev-julius",
    );
  });
});
...
describe("managed endpoint names", () => {
  it("uses the stage slug and a stable stage-scoped digest suffix", () => {
    ...
    expect(managedEndpointHostname("prod", "vipercoderelay.com", hash)).toBe(
      "prod-abcdef0123456789.vipercoderelay.com",
    );
    expect(managedEndpointTunnelName("dev_julius", hash)).toBe(
      "vipercoderelay-managedendpoint-dev-julius-abcdef0123456789",
    );
  });
