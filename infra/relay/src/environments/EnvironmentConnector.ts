...
import { makeEnvironmentHttpApiClient } from "@vipercode/client-runtime";
import {
...
>()("vipercode-relay/environments/EnvironmentConnector") {}
...
function verifyEnvironmentResponse(input: {
  ...
  return verifyWithEnvironmentKeys({
    ...
    issuer: `viper-env:${input.environmentId}`,
...
const make = Effect.gen(function* () {
  ...
  return EnvironmentConnector.of({
    status: Effect.fn("relay.environment_connector.status")(function* (input) {
      ...
      const payload = {
        iss: relayIssuer,
        aud: `viper-env:${link.environmentId}`,
        sub: input.userId,
