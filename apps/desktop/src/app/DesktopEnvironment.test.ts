...
const defaultInput = {
  ...
  processArch: "arm64",
  appVersion: "0.0.22",
  appPath: "/Applications/Viper Code.app/Contents/Resources/app.asar",
  isPackaged: false,
  resourcesPath: "/Applications/Viper Code.app/Contents/Resources",
...
describe("DesktopEnvironment", () => {
  it.effect("derives state paths and development identity inside Effect", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          VIPERCODE_HOME: " /tmp/viper ",
          VIPERCODE_COMMIT_HASH: " 0123456789abcdef ",
          VIPERCODE_PORT: "4949",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
          VIPERCODE_DEV_REMOTE_VIPER_SERVER_ENTRY_PATH: " /remote/server.mjs ",
          VIPERCODE_OTLP_TRACES_URL: " http://127.0.0.1:4318/v1/traces ",
          VIPERCODE_OTLP_EXPORT_INTERVAL_MS: "2500",
        },
      );

      assert.equal(environment.isDevelopment, true);
      assert.equal(environment.appDataDirectory, "/Users/alice/Library/Application Support");
      assert.equal(environment.baseDir, "/tmp/viper");
      ...
      assert.equal(environment.appUserModelId, "com.vipercode.vipercode.dev");
      assert.equal(environment.linuxWmClass, "vipercode-dev");
      assert.deepEqual(
  ...
  it.effect("derives production state paths under userdata", () =>
    Effect.gen(function* () {
      ...
      assert.equal(environment.stateDir, "/tmp/viper/userdata");
      assert.equal(environment.logDir, "/tmp/viper/userdata/logs");
      assert.equal(environment.serverSettingsPath, "/tmp/viper/userdata/settings.json");
    }),
  );
  ...
  it.effect("uses a configured app user model id override", () =>
    Effect.gen(function* () {
      const environment = yield* makeEnvironment(
        {},
        {
          VIPERCODE_DESKTOP_APP_USER_MODEL_ID: " com.vipercode.vipercode.dev.local ",
          VITE_DEV_SERVER_URL: "http://localhost:5173",
        },
      );

      assert.equal(environment.appUserModelId, "com.vipercode.vipercode.dev.local");
    }),
  );
