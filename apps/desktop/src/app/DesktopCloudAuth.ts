...
export const CLOUD_AUTH_CALLBACK_SCHEME = "vipercode";
export const DEVELOPMENT_CLOUD_AUTH_CALLBACK_SCHEME = "vipercode-dev";
...
export class DesktopCloudAuth extends Context.Service<DesktopCloudAuth, DesktopCloudAuthShape>()(
  "@vipercode/desktop/app/DesktopCloudAuth",
) {}
...
function resolveConfiguredProtocolClient(): {
  ...
  const path = process.env.VIPERCODE_DESKTOP_PROTOCOL_CLIENT_PATH?.trim();
  if (!path) return null;

  return {
    path,
    args: (process.env.VIPERCODE_DESKTOP_PROTOCOL_CLIENT_ARGS ?? "")
      .split("\n")
      .map((arg) => arg.trim())
...
function isProtocolRegistrationManagedExternally(): boolean {
  return process.env.VIPERCODE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED?.trim() === "1";
}

function resolveProtocolCallbackForwardUrl(): URL | null {
  const rawUrl = process.env.VIPERCODE_DESKTOP_PROTOCOL_CALLBACK_URL?.trim();
