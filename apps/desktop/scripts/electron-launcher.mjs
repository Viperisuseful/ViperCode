// This file mostly exists because we want dev mode to say "Viper Code (Dev)" instead of "electron"
...
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["vipercode-dev"] : ["vipercode"];
...
function resolveDevelopmentProtocolCallbackPort() {
  const configuredPort = Number.parseInt(process.env.VIPERCODE_PORT ?? "", 10);
...
function writeDevelopmentLauncherScript(targetBinaryPath, electronBinaryPath) {
  ...
  const envEntries = [
    ...
    ["VIPERCODE_PORT", process.env.VIPERCODE_PORT],
    ["VIPERCODE_HOME", process.env.VIPERCODE_HOME],
    ["VIPERCODE_COMMIT_HASH", process.env.VIPERCODE_COMMIT_HASH],
    ["VIPERCODE_OTLP_TRACES_URL", process.env.VIPERCODE_OTLP_TRACES_URL],
    ["VIPERCODE_OTLP_EXPORT_INTERVAL_MS", process.env.VIPERCODE_OTLP_EXPORT_INTERVAL_MS],
    ["VIPERCODE_DESKTOP_APP_USER_MODEL_ID", APP_BUNDLE_ID],
    ["VIPERCODE_DESKTOP_PROTOCOL_REGISTRATION_MANAGED", "1"],
    ["VIPERCODE_DESKTOP_PROTOCOL_CALLBACK_URL", protocolCallbackUrl],
  ...
  writeFileSync(
    ...
    [
      ...
      '      if [ -n "$VIPERCODE_DESKTOP_PROTOCOL_CALLBACK_URL" ]; then',
      '        /usr/bin/curl -fsS --max-time 2 -X POST --data-binary "$arg" "$VIPERCODE_DESKTOP_PROTOCOL_CALLBACK_URL" >/dev/null 2>&1 && exit 0',
      "      fi",
      "      ;;",
      "  esac",
      "done",
      `exec ${shellSingleQuote(electronBinaryPath)} --vipercode-dev-root=${shellSingleQuote(desktopDir)} ${shellSingleQuote(mainEntryPath)} "$@"`,
      "",
    ].join("\n"),
  );
