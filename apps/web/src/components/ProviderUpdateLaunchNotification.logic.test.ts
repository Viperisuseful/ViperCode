...
describe("provider update launch notification logic", () => {
  ...
  it("builds a notification key from provider latest versions", () => {
    ...
    const grok = updateCandidate({
      driver: driver("grok"),
      version: "0.2.0",
      latestVersion: "0.3.0",
    });

    expect(providerUpdateNotificationKey([codex, grok])).toBe("codex:1.1.0|grok:0.3.0");
  ...
  it("describes settings-only updates without one-click support", () => {
    const view = getProviderUpdateInitialToastView({
      updateProviders: [
        ...
        updateCandidate({ driver: driver("grok"), canUpdate: false }),
      ],
      oneClickProviders: [],
    });

    expect(view.description).toBe("Codex and Grok can be updated from provider settings.");
  });
  ...
  it("keeps unchanged providers actionable from settings", () => {
    ...
    expect(view).toMatchObject({
      ...
      description: "Grok still appears outdated. Check provider settings for details.",
    });
  });
  ...
  it("collects only attempted provider snapshots from update responses", () => {
    ...
    const grok = provider({ driver: driver("grok") });
    const results: PromiseSettledResult<{ readonly providers: ReadonlyArray<ServerProvider> }>[] = [
      { status: "fulfilled", value: { providers: [codex, grok] } },
    ];

    expect(
      collectUpdatedProviderSnapshots({
        results,
        providerInstanceIds: new Set([grok.instanceId]),
      }),
    ).toEqual([grok]);
  });
