...
import { RelayOkResponse } from "@vipercode/contracts/relay";
...
function formatCloudStatus(status: CloudCliStatus, options?: { readonly json?: boolean }): string {
  ...
  const nextStep = !status.authenticated
    ? "Run `viper connect link` to authorize and enable Viper Connect."
    : !status.desired
      ? "Run `viper connect link` to enable Viper Connect."
      : !status.linked
        ? "Start Viper Code to provision the environment link and launch its managed tunnel."
        : undefined;
...
const confirmRelayClientInstall = (version: string) =>
  Prompt.run(
    Prompt.confirm({
      message: `The Viper relay client is required for Viper Connect. Download and install version ${version}?`,
      initial: false,
    }),
  );
...
const disconnectCloud = Effect.fn("cloud.cli.disconnect")(function* (options: {
  ...
  if (liveResult.status === "failed") {
    yield* Console.warn(
      `Viper Connect is disabled, but the running server could not stop its tunnel: ${String(liveResult.cause)}\nRestart that server to stop the connector.`,
    );
  } else {
    yield* Console.log("Viper Connect is disabled locally.");
  }
  ...
  if (options.clearAuthorization) {
    yield* Console.log("Signed out of Viper Connect locally.");
  }
});
...
const connectLoginCommand = Command.make("login", {
  ...
  Command.withDescription("Authorize the Viper Connect CLI without enabling remote access."),
  Command.withHandler((flags) =>
    runCloudCommand(
      ...
      Effect.gen(function* () {
        const tokens = yield* CliTokenManager.CloudCliTokenManager;
        yield* tokens.get;
        yield* Console.log("Signed in to Viper Connect.");
      }),
    ),
  ),
);
...
const connectLinkCommand = Command.make("link", {
  ...
  Command.withDescription("Authorize this environment for Viper Connect on next start."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
        ...
        if (Option.isNone(installed)) {
          yield* Console.log("Viper Connect setup cancelled. The relay client was not installed.");
          return;
        }
        yield* Console.log(
        ...
        yield* Console.log(
          "This Viper environment will be available through Viper Connect the next time Viper Code starts.",
        );
      }),
    ),
  ),
...
const connectStatusCommand = Command.make("status", {
  ...
  Command.withDescription("Show persisted Viper Connect and relay client state."),
  Command.withHandler((flags) =>
    runCloudCommand(
      flags,
      Effect.gen(function* () {
...
const connectUnlinkCommand = Command.make("unlink", {
  ...
  Command.withDescription("Disable Viper Connect while retaining the stored authorization."),
  Command.withHandler((flags) =>
...
const connectLogoutCommand = Command.make("logout", {
  ...
  Command.withDescription("Disable Viper Connect and clear the stored CLI authorization."),
  Command.withHandler((flags) =>
...
export const connectCommand = Command.make("connect").pipe(
  Command.withDescription("Manage headless Viper Connect access."),
