...
import { RELAY_HEALTH_REQUEST_TYP, RELAY_MINT_REQUEST_TYP } from "@vipercode/shared/relayJwt";
  ...
  Effect.gen(function* () {
    ...
    const runtime = ManagedRuntime.make(
      OtlpTracer.layer({
        ...
        resource: {
          serviceName: "viper-web",
          attributes: {
            "service.runtime": "viper-web",
...
const buildAppUnderTest = (options?: {
  ...
  Effect.gen(function* () {
    ...
    const tempBaseDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "viper-router-test-" });
    ...
    const appLayer = servedRoutesLayer.pipe(
      ...
      Layer.provide(
        Layer.mock(CloudCliTokenManager.CloudCliTokenManager)({
          get: Effect.die(new Error("Unexpected Viper Connect CLI authorization request.")),
...
const makeCloudMintCredentialRequest = (input: {
  ...
  const payload = {
    ...
    aud: input.audience ?? `viper-env:${input.environmentId}`,
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  it.effect("serves static index content for GET / when staticDir is configured", () =>
    Effect.gen(function* () {
      ...
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "viper-router-static-" });
  ...
  it.effect("serves project favicon requests before the dev URL redirect", () =>
    Effect.gen(function* () {
      ...
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "viper-router-project-favicon-",
      });
  ...
  it.effect("serves the fallback project favicon when no icon exists", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "viper-router-project-favicon-fallback-",
      });
  ...
  it.effect("serves the public environment descriptor without requiring auth", () =>
    Effect.gen(function* () {
      ...
      const url = yield* getHttpServerUrl("/.well-known/viper/environment");
      ...
      Effect.gen(function* () {
        ...
        const tokenResponse = yield* fetchEffect(tokenUrl, {
          ...
          body: new URLSearchParams({
            ...
            subject_token_type: "urn:viper:params:oauth:token-type:environment-bootstrap",
    ...
    () =>
      Effect.gen(function* () {
        const installedRelayClient = {
          ...
          executablePath: "/tmp/viper/tools/cloudflared",
  ...
  it.effect("serves the documented Viper Connect mint credential endpoint", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const mintUrl = yield* getHttpServerUrl("/api/viper-connect/mint-credential");
  ...
  it.effect("serves signed Viper Connect environment health checks", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
      ...
      const healthUrl = yield* getHttpServerUrl("/api/viper-connect/health");
    ...
    Effect.gen(function* () {
      ...
      const wrongAudience = yield* postMint(
        makeCloudMintCredentialRequest({
          ...
          environmentId: testEnvironmentDescriptor.environmentId,
          clientProofKeyThumbprint: "client-proof-key-thumbprint",
          audience: "viper-env:other-environment",
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const projectDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "viper-router-project-favicon-query-token-",
        });
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "viper-web");
  ...
  it.effect("rejects websocket rpc handshake when session authentication is missing", () =>
    Effect.gen(function* () {
      ...
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "viper-ws-auth-required-" });
  ...
  it.effect("routes websocket rpc projects.searchEntries", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "viper-ws-project-search-" });
      yield* fs.writeFileString(
        path.join(workspaceDir, "needle-file.ts"),
        "export const needle = 1;",
      );
  ...
  it.effect("routes websocket rpc projects.searchEntries excludes gitignored files", () =>
    Effect.gen(function* () {
      ...
      const workspaceDir = yield* fs.makeTempDirectoryScoped({
        prefix: "viper-ws-project-search-gitignored-",
      });
  ...
  it.effect("routes websocket rpc projects.writeFile", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const workspaceDir = yield* fs.makeTempDirectoryScoped({ prefix: "viper-ws-project-write-" });
  ...
  it.effect("creates a missing workspace root during websocket project.create dispatch", () =>
    Effect.gen(function* () {
      const fs = yield* FileSystem.FileSystem;
      const path = yield* Path.Path;
      const parentDir = yield* fs.makeTempDirectoryScoped({ prefix: "viper-ws-project-create-" });
  ...
  it.effect("routes websocket rpc shell.openInEditor", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(openedInput, { cwd: "/tmp/project", editor: "vscode" });
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  it.effect("enriches replayed project events with repository identity metadata", () =>
    Effect.gen(function* () {
      const repositoryIdentity = {
        canonicalKey: "github.com/vipercode/vipercode",
        locator: {
          source: "git-remote" as const,
          remoteName: "origin",
          remoteUrl: "git@github.com:ViperCode/vipercode.git",
        },
        displayName: "ViperCode/vipercode",
        provider: "github",
        owner: "ViperCode",
        name: "vipercode",
      };
      ...
      Effect.gen(function* () {
        ...
        const refreshStatus = vi.fn((_: string) =>
          Effect.succeed({
            ...
            hasPrimaryRemote: true,
            isDefaultRef: false,
            refName: "vipercode/bootstrap-refName",
            hasWorkingTreeChanges: false,
            workingTree: {
              files: [],
              insertions: 0,
            ...
            client[ORCHESTRATION_WS_METHODS.dispatchCommand]({
              ...
              bootstrap: {
                ...
                prepareWorktree: {
                  ...
                  baseBranch: "main",
                  branch: "vipercode/bootstrap-refName",
                },
                runSetupScript: true,
              },
              createdAt,
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          cwd: "/tmp/project",
          refName: "main",
          newRefName: "vipercode/bootstrap-refName",
          path: null,
        });
