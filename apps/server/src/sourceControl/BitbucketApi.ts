...
import { sanitizeBranchFragment } from "@vipercode/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@vipercode/shared/sourceControl";
...
const BitbucketApiEnvConfig = Config.all({
  baseUrl: Config.string("VIPERCODE_BITBUCKET_API_BASE_URL").pipe(
    Config.withDefault(DEFAULT_API_BASE_URL),
  ),
  accessToken: Config.string("VIPERCODE_BITBUCKET_ACCESS_TOKEN").pipe(Config.option),
  email: Config.string("VIPERCODE_BITBUCKET_EMAIL").pipe(Config.option),
  apiToken: Config.string("VIPERCODE_BITBUCKET_API_TOKEN").pipe(Config.option),
});
...
export class BitbucketApi extends Context.Service<BitbucketApi, BitbucketApiShape>()(
  "viper/sourceControl/BitbucketApi",
) {}
...
function checkoutBranchName(input: {
  ...
  return `vipercode/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
...
function authFromConfig(
  ...
  return {
    ...
    host: Option.some("bitbucket.org"),
    detail: Option.some(
      "Set VIPERCODE_BITBUCKET_EMAIL and VIPERCODE_BITBUCKET_API_TOKEN, or VIPERCODE_BITBUCKET_ACCESS_TOKEN.",
    ),
  };
}
