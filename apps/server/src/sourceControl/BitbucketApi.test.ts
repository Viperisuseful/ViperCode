...
const bitbucketPullRequest = {
  ...
  links: {
    html: {
      href: "https://bitbucket.org/Viperisuseful/ViperCode/pull-requests/42",
    },
  },
  source: {
    branch: { name: "feature/source-control" },
    repository: {
      full_name: "octocat/vipercode",
  ...
  destination: {
    branch: { name: "main" },
    repository: {
      full_name: "Viperisuseful/ViperCode",
      workspace: { slug: "viperisuseful" },
    },
  },
};
...
const repositoryJson = {
  full_name: "Viperisuseful/ViperCode",
  links: {
    html: { href: "https://bitbucket.org/Viperisuseful/ViperCode" },
    clone: [
      { name: "https", href: "https://bitbucket.org/Viperisuseful/ViperCode.git" },
      { name: "ssh", href: "git@bitbucket.org:Viperisuseful/ViperCode.git" },
    ],
  },
...
function makeLayer(input: {
  ...
  const gitMock = {
    readConfigValue: vi.fn<GitVcsDriver.GitVcsDriverShape["readConfigValue"]>(() =>
      Effect.succeed<string | null>("git@bitbucket.org:Viperisuseful/ViperCode.git"),
    ),
    ...
    listRemotes: () =>
      Effect.succeed({
        remotes: [
          {
            name: "origin",
            url: "git@bitbucket.org:Viperisuseful/ViperCode.git",
            pushUrl: Option.none(),
            isPrimary: true,
          },
        ],
    ...
    Layer.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            VIPERCODE_BITBUCKET_API_BASE_URL: "https://api.test.local/2.0",
            VIPERCODE_BITBUCKET_EMAIL: "user@example.com",
            VIPERCODE_BITBUCKET_API_TOKEN: "token",
          },
        }),
      ),
    ),
...
it.effect("parses pull request responses from the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(result, {
      number: 42,
      title: "Add Bitbucket provider",
      url: "https://bitbucket.org/Viperisuseful/ViperCode/pull-requests/42",
      baseRefName: "main",
      headRefName: "feature/source-control",
      state: "open",
      updatedAt: Option.some(DateTime.makeUnsafe("2026-01-02T00:00:00.000Z")),
      isCrossRepository: true,
      headRepositoryNameWithOwner: "octocat/vipercode",
      headRepositoryOwnerLogin: "octocat",
    });
    assert.strictEqual(
      execute.mock.calls[0]?.[0].url,
      "https://api.test.local/2.0/repositories/Viperisuseful/ViperCode/pullrequests/42",
    );
...
it.effect("lists pull requests with Bitbucket state and source branch query params", () => {
        ...
        values: [
          {
            ...
            source: {
              ...
              repository: { full_name: "Viperisuseful/ViperCode" },
            },
          },
        ],
      }),
  ...
  return Effect.gen(function* () {
    ...
    assert.strictEqual(
      request?.url,
      "https://api.test.local/2.0/repositories/Viperisuseful/ViperCode/pullrequests",
    );
...
it.effect("reads repository clone URLs and default branch", () => {
  ...
  return Effect.gen(function* () {
    ...
    const cloneUrls = yield* bitbucket.getRepositoryCloneUrls({
      cwd: "/repo",
      repository: "Viperisuseful/ViperCode",
    });
    const defaultBranch = yield* bitbucket.getDefaultBranch({ cwd: "/repo" });

    assert.deepStrictEqual(cloneUrls, {
      nameWithOwner: "Viperisuseful/ViperCode",
      url: "https://bitbucket.org/Viperisuseful/ViperCode.git",
      sshUrl: "git@bitbucket.org:Viperisuseful/ViperCode.git",
    });
  ...
  () => {
    ...
    return Effect.gen(function* () {
      ...
      assert.deepStrictEqual(
        ...
        [
          "https://api.test.local/2.0/repositories/Viperisuseful/ViperCode",
          "https://api.test.local/2.0/repositories/Viperisuseful/ViperCode/branching-model",
        ].toSorted(),
      );
    }).pipe(Effect.provide(layer));
  },
...
it.effect("creates repositories through the Bitbucket REST API", () => {
  ...
  return Effect.gen(function* () {
    ...
    const request = execute.mock.calls[0]?.[0];
    assert.strictEqual(request?.url, "https://api.test.local/2.0/repositories/Viperisuseful/ViperCode");
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(JSON.parse(new TextDecoder().decode(rawBody)), {
      ...
      source: {
        ...
        repository: { full_name: "owner/vipercode" },
      },
      destination: {
        branch: { name: "main" },
      },
  ...
  return Effect.gen(function* () {
    ...
    yield* bitbucket.checkoutPullRequest({
      ...
      context: {
        ...
        remoteName: "origin",
        remoteUrl: "git@bitbucket.org:Viperisuseful/ViperCode.git",
      },
      reference: "42",
      force: true,
    });
...
it.effect("checks out fork pull requests through an ensured fork remote", () => {
  const { git, layer } = makeLayer({
    response: (request) => {
      if (request.url.endsWith("/repositories/octocat/vipercode")) {
        return Response.json({
          ...repositoryJson,
          full_name: "octocat/vipercode",
          links: {
            html: { href: "https://bitbucket.org/octocat/vipercode" },
            clone: [
              { name: "https", href: "https://bitbucket.org/octocat/vipercode.git" },
              { name: "ssh", href: "git@bitbucket.org:octocat/vipercode.git" },
            ],
          },
        });
      }
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(git.ensureRemote.mock.calls[0]?.[0], {
      ...
      url: "git@bitbucket.org:octocat/vipercode.git",
    });
    assert.deepStrictEqual(git.fetchRemoteBranch.mock.calls[0]?.[0], {
      cwd: "/repo",
      remoteName: "octocat",
      remoteBranch: "main",
      localBranch: "vipercode/pr-42/main",
    });
    assert.deepStrictEqual(git.setBranchUpstream.mock.calls[0]?.[0], {
      cwd: "/repo",
      branch: "vipercode/pr-42/main",
      remoteName: "octocat",
      remoteBranch: "main",
    });
    assert.deepStrictEqual(git.switchRef.mock.calls[0]?.[0], {
      cwd: "/repo",
      refName: "vipercode/pr-42/main",
    });
  }).pipe(Effect.provide(layer));
});
