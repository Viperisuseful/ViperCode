...
import { GitCommandError, type SourceControlProviderError } from "@vipercode/contracts";
...
const CLONE_URLS = {
  ...
  url: "https://github.com/octocat/vipercode",
  sshUrl: "git@github.com:octocat/vipercode.git",
};
...
function makeLayer(input: {
  ...
  return SourceControlRepositoryService.layer.pipe(
    ...
    Layer.provide(ServerConfig.layerTest(process.cwd(), { prefix: "viper-source-control-repos-" })),
...
it.effect("looks up repositories through the requested provider without search", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(calls, [{ cwd: "/workspace", repository: "octocat/vipercode" }]);
...
it.effect("clones a looked-up repository into the requested destination", () =>
  Effect.gen(function* () {
    ...
    const parent = yield* fs.makeTempDirectoryScoped({
      prefix: "viper-source-control-clone-parent-",
    });
    const destinationPath = `${parent}/vipercode`;
    ...
    yield* Effect.gen(function* () {
      ...
      assert.deepStrictEqual(cloneCalls, [
        {
          cwd: parent,
          args: ["clone", CLONE_URLS.url, "vipercode"],
        },
      ]);
    }).pipe(
      Effect.provide(
...
it.effect("publishes by creating the repository, adding a remote, and pushing upstream", () => {
  ...
  return Effect.gen(function* () {
    ...
    assert.deepStrictEqual(createCalls, [
      { cwd: "/workspace", repository: "octocat/vipercode", visibility: "private" },
    ]);
