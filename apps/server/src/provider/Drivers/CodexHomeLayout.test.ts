...
import { CodexSettings } from "@vipercode/contracts";
import {
...
it.layer(NodeServices.layer)("CodexHomeLayout", (it) => {
  describe("resolveCodexHomeLayout", () => {
    it.effect("uses direct CODEX_HOME when no shadow home is configured", () =>
      Effect.gen(function* () {
        const homePath = yield* makeTempDir("vipercode-codex-home-");
    ...
    it.effect("uses the shared home for continuation and the shadow home for runtime", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const sharedHome = yield* makeTempDir("vipercode-codex-shared-");
        const shadowRoot = yield* makeTempDir("vipercode-codex-shadow-root-");
