...
import { EnvironmentOrchestrationHttpApi } from "@vipercode/contracts";
...
it.layer(NodeServices.layer)("bin cli parsing", (it) => {
  ...
  it.effect("rejects connect commands when public configuration is missing", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["viper", "connect"]);
      assert.include(error.errors[0]?.message ?? "", "missing Viper Connect public configuration");

      const output = (yield* TestConsole.errorLines).join("\n");
      assert.include(output, "ERROR");
      assert.include(output, "missing Viper Connect public configuration");
    }).pipe(Effect.provide(Layer.mergeAll(CliRuntimeLayer, TestConsole.layer))),
  );

  it.effect("reports fresh headless connect state without requiring local configuration", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "viper-cli-cloud-status-test-"));
  ...
  it.effect("reports actionable human-readable headless connect state", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "viper-cli-cloud-status-human-test-"));
      const { output } = yield* captureStdout(
  ...
  it.effect("logs in to headless connect without enabling access", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "viper-cli-cloud-login-test-"));
      ...
      assert.equal(login.output, "Signed in to Viper Connect.");
  ...
  it.effect("disables headless connect without a running server", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "viper-cli-cloud-unlink-test-"));
      const { output } = yield* captureStdout(
        runConnectCli(["connect", "unlink", "--base-dir", baseDir]),
      );

      assert.equal(output, "Viper Connect is disabled locally.");
    }),
  );

  it.effect("logs out of headless connect and removes the stored CLI authorization", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "viper-cli-cloud-logout-test-"));
      const { secretsDir } = yield* deriveServerPaths(baseDir, undefined);
      ...
      assert.equal(output, "Signed out of Viper Connect locally.");
  ...
  it.effect("executes auth pairing subcommands and redacts secrets from list output", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "viper-cli-auth-pairing-test-"));
  ...
  it.effect("executes auth session subcommands and redacts secrets from list output", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "viper-cli-auth-session-test-"));
  ...
  it.effect("rejects invalid ttl values before running auth commands", () =>
    Effect.gen(function* () {
      ...
      assert.deepEqual(error.commandPath, ["viper", "auth", "pairing", "create"]);
  ...
  it.effect("adds, renames, and removes projects offline through the orchestration engine", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "viper-cli-projects-offline-test-"));
      const workspaceRoot = mkdtempSync(join(tmpdir(), "viper-cli-projects-workspace-"));
  ...
  it.effect("routes project commands through a running server when runtime state is present", () =>
    Effect.gen(function* () {
      const baseDir = mkdtempSync(join(tmpdir(), "viper-cli-projects-live-test-"));
      const workspaceRoot = mkdtempSync(join(tmpdir(), "viper-cli-projects-live-workspace-"));
  ...
  it.effect("rejects dev-url on project commands", () =>
    Effect.gen(function* () {
      const workspaceRoot = mkdtempSync(
        join(tmpdir(), "viper-cli-projects-unknown-option-workspace-"),
      );
      ...
      assert.deepEqual(error.commandPath, ["viper", "project", "add"]);
